    ...
    show: function (el, options) {
        ...
        el.ttimer = setTimeout(function () {
            ...
            var st = fix ? 0 : (bodyNode.scrollTop || htmlNode.scrollTop || 0),
                ...
                needUp = (xy[1] + elsize[1] + ttsize[1] + opts.shift[2] - windowHeight()) > st;
            if (browser.msie7) {
                setStyle(container, {
                    width: ttsize[0]
                });
            }
            var needLeft = (opts.black && windowWidth() && windowWidth() - (xy[0] + ttsize[0]) < 1);
            if (toup && needDown) {
                toup = false;
            ...
            if (opts.center && ttsize[0] != elsize[0]) {
                ...
                if (pointer) {
                    setStyle(pointer, {
                        marginLeft: ((ttsize[0] - getSize(pointer)[0]) / 2 || 0) + 'px'
                    });
                }
            }
